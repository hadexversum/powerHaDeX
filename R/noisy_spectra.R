...
#'
#' @description This function creates a list of lists of noisy deuteration
#' curves based on theoretical spectra in order to imitate the data from the
#' HDX experiments.
#'
#' @param theoretical_spectra a data table or a list of data tables of
...
#' @param n_experiments number of replicates of an experiment for power
#' calculation.
#' @param mass_deviations mass deviation in parts per million. Either a single
...
#' Default to 50.
#' @param intensity_deviations optional, standard deviations of random noise
...
#' @param relative logical, if TRUE (default), each deuteration curve will start
#' at 0 (relative mass will be returned). Default \code{TRUE}.
#'
#' @return a list (for paired states when \code{compare_pairs} is \code{TRUE})
...
get_noisy_deuteration_curves <- function(theoretical_spectra,
    ...
    spectra <- add_noise_to_spectra(spectra, n_replicates, n_experiments,
                                    undeuterated_mass, mass_deviations,
...
#'
#' @return list of data.tables containing spectra - for paired states or all
#' states.
#'
#' @export
get_spectra_list <- function(theoretical_spectra,
    ...
    if (compare_pairs & (is.na(reference))) stop("With pair comparisons,
                                                 reference protection factor
                                                 must be provided")
    ...
    if (compare_pairs) {
        ...
        theoretical_spectra[["Experimental_state"]] <- paste0("State_",
...
#'
#' @description A supplementary function for
#' \code{\link[powerHaDeX]{get_spectra_list}}.
#'
#' @inheritParams get_noisy_deuteration_curves
#'
...
#' @param undeuterated_mass the value of mass of an undeuterated peptide
#' computed by \code{\link[powerHaDeX]{get_undeuterated_mass}}.
#'
#' @return list of lists of data.tables
#'
#' @details This function uses
#' \code{\link[powerHaDeX]{make_experimental_design}} and
...
#' Creates spectra with technical replicates and noise
#'
#' @param spectra a list of spectra obtained via
...
#'
#' @description This function adds noise to intensities and/or masses in a
#' spectrum.
#'
...
add_noise_to_one_spectrum <- function(spectrum,
    ...
    if (length(mass_deviations) == 1) {
        mass_deviations <- rep(mass_deviations,
    ...
    spectrum <- spectrum[, add_noise_to_one_timepoint(.SD, sds),
                         by = "Exposure",
    ...
    if (!is.null(intensity_deviations)) {
        ...
        spectrum <- spectrum[, add_noise_to_intensities(.SD, sds),
                             by = "Exposure"]
    }
...
#'
#' @details The centroid mass value from spectrum is calculated as a weighted
...
#'
#' @description This function makes noisy deuteration curves for power
...
add_noise_to_curves <- function(curves,
    ...
    lapply(curves, function(curve) {
        ...
        lapply(curve, function(replicate_curve) {
            ...
            add_noise_to_single_curve(replicate_curve,
                                      per_run_deviations,
                                      relative)
...
#'
#' @param replicate_curve `data.table` with technical replicates of a
...
add_noise_to_single_curve <- function(replicate_curve,
    ...
    if (!is.null(per_run_deviations)) {
        ...
        if (length(per_run_deviations) == 1) {
            ...
            per_run_deviations <- rep(per_run_deviations,
                                      length(unique(replicate_curve[["Rep"]])))
        }
        names(per_run_deviations) <- as.character(unique(replicate_curve[["Rep"]]))

        replicate_curve <- replicate_curve[,
                                           add_noise(.SD, per_run_deviations),
                                           by = "Rep",
                                           .SDcols = colnames(replicate_curve)]
...
#'
#' @description This function gets mass of an undeuterated peptide based on its
#' spectrum.
#'
#' @inheritParams get_noisy_deuteration_curves
#'
...
#'
#' @return `data.table` of calculated mass value for the first peak (the
...
#' \code{Mass} values unless they are not zeroes (there is no noise at the time
#' \code{0}).
#'
#' @keywords internal
#'
add_noise <- function(replicate_curve, standard_deviations) {
    ...
    replicate_curve[["Mass"]] <- ifelse(replicate_curve[["Mass"]] == 0,
                                        0,
                                        replicate_curve[["Mass"]] +
                                            rnorm(nrow(replicate_curve),0,sd = sd))
    replicate_curve
}
...
#' and standard deviation equal to \code{standard_deviations} and added to
#' \code{Mz} values for the time points (based on supplied parameters for time
#' points).
#'
#' @keywords internal
#'
...
add_noise_to_intensities <- function(spectrum, standard_deviations) {
    ...
    spectrum[["Intensity"]] <- spectrum[["Intensity"]] +
        rnorm(nrow(spectrum), 0, standard_deviations)
...
#' Get relative mass
#'
#' @description Mass minus mass at zero is calculated as a form of deuterium
#' uptake.
#'
