...
$$pKc = -\log_{10}\left(10^{E_{const}} \cdot \exp\Bigg\lbrace \dfrac{-E_a}{gas\_constant}\left( \dfrac{1}{temp\_kelvin} - \dfrac{1}{278}\right)\Bigg\rbrace\right)$$
...
$$const = \log_{10}\left( \dfrac{10^{c_1 - pH} + 10^{c_2 - pKc}}{10^{-pKc} + 10^{-pH}}\right)$$
...
The data obtained by the procedure described above is called the theoretical spectrum. Such a spectrum is the first step of the simulation shown in the first diagram when simulated for two biological states and different time points.
...
- *n_runs* - number of technical replicates to create.

- *n_replicates* - number of replicates of curves for power calculation.
...
$$\dfrac{mass\_deviations\cdot\texttt{undeuterated\_mass}}{10^6}$$
...
Noisy spectra are obtained from the function *add_noise_to_spectra*. It takes the parameters *spectra* (the output of  *get_spectra_list*), *n_runs*, *n_replicates*, *undeuterated_mass* (calculated by *get_undeuterated_mass*), *mass_deviations* and *intensity_deviations* (those parameters are described before).
