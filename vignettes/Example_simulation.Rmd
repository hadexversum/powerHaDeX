...
The package `powerHaDeX` is a tool for simulating and analyzing data coming from HDX-MS experiments along with the possibility of comparing the power of the tests verifying differences in deuteration levels. The functionality of the simulation involves generating theoretical spectrum along with its replications, replicating the deuterium uptake curves obtained from spectra and rejection rate estimation for comparison of the tests for differences in deuteration. Let us go through the process of simulation and all its features.
...
We can use the function `calculate_hdx_power` to execute testing procedure on the data simulated by `get_noisy_deuteration_curves`. An example usage of the function is shown below
